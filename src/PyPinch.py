...
class Streams:
    def __init__(self, streamsData_df: pd.DataFrame, deltaT_min: float):
        self.tmin = 0
        self.numberOf = 0
        self.streamsData = []
        self.tmin = deltaT_min

        self._rawStreamsData = []
        self._index = 0
        self._length = 0

        self._rawStreamsData = streamsData_df.values.tolist()
...
class PyPinch:
    def __init__(self, streamsData: list[StreamData], deltaT_min: float, options={}):
        self.tmin = 0
        self.streams = []
        ...
        self._deltaHHot = []
        self._deltaHCold = []
        self._options = {"debug": False, "draw": False, "csv": False, "return_results": False}
        streamsData_df = pd.DataFrame(
            [[s.CP, s.TSUPPLY, s.TTARGET] for s in streamsData], columns=["CP", "TSUPPLY", "TTARGET"]
        )

        self.streams = Streams(streamsData_df, deltaT_min)
        ...
        if "return_results" in options:
            self._options["return_results"] = True
    ...
    def constructTemperatureInterval(self):
        ...
        if self._options["debug"]:
            ...
            for interval in self.temperatureInterval:
                ...
                i = i + 1
        if self._options["return_results"]:
            self.returned_results += "\nTemperature Intervals: \n"
            i = 0
            for interval in self.temperatureInterval:
                self.returned_results += "Interval {} : {}\n".format(i, interval)
                i = i + 1
    ...
    def drawProblemTable(self):
        ...
        colLabels = [
            r"$Interval: S_i - S_{i+1}$",
            r"$\Delta S (\degree C)$",
            r"$\Delta CP (kW / \degree C)$",
            r"$\Delta H (kW)$",
            "",
        ]
        cellText = []
    ...
    def constructHeatCascade(self):
        ...
        if self._options["return_results"]:
            ...
            self.returned_results += "\nPinch Temperature (degC): {}\n".format(
                self.pinchTemperature
            )
            self.returned_results += "Minimum Hot Utility (kW): {}\n".format(
                self.hotUtility
            )
            self.returned_results += "Minimum Cold Utility (kW): {}\n".format(
                self.coldUtility
            )
    ...
    def drawHeatCascade(self):
        ...
        cellText.append(["Interval", r"$\Delta H (kW)$", "Exit H (total kW)"])
    ...
    def constructShiftedCompositeDiagram(self):
        ...
        if self._options["return_results"]:
            self.returned_results += "\nShifted Composite Diagram Values: \n"
            self.returned_results += "DeltaH Hot: {}\n".format(self._deltaHHot)
            self.returned_results += "DeltaH Cold: {}\n".format(self._deltaHCold)

            self.returned_results += "\nHot H: {}\n".format(
                self.shiftedCompositeDiagram["hot"]["H"]
            )
            self.returned_results += "Hot T: {}\n".format(
    ...
    def constructGrandCompositeCurve(self):
        ...
        if self._options["return_results"]:
            self.returned_results += "\nGrand Composite Curve: \n"
            self.returned_results += "Net H (kW): {}\n".format(self.grandCompositeCurve["H"])
            self.returned_results += "T (degC): {}\n".format(self.grandCompositeCurve["T"])
